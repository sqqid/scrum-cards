...
- **[Backend](scrum_card_backend/)** — Go, standard library only. Rooms live in
  memory, and every change is pushed to the whole room over server-sent events.
  No web framework, no database, no third-party runtime dependencies.
- **[Frontend](scrum_cards_ui/)** — React and TypeScript, bundled with Vite.
  RxJS drives the live parts, and the styling is plain BEM CSS with light/dark
  theming.
